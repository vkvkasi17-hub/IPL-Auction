...
import {players,money,type Game} from '@/lib/game';
...
export function Celebration({game,myId}:{game:Game;myId?:string}){
 ...
 useEffect(()=>{
  ...
  if(fresh.length){const won=fresh.at(-1)!;const earlier=Object.values(game.seats).flatMap(s=>s.squad).filter(s=>s.player<won.player);setAward({...won,record:earlier.length>0&&won.price>Math.max(...earlier.map(s=>s.price))})}
 },[game,myId]);
